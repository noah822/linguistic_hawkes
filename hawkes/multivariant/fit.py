...
from hawkes._jit_hotspot import bundled_g_lag_compute, kernel_est_on_window
from hawkes._internal import (
    normalize,
    pairwise_difference,
    definite_integral,
)
from hawkes.dist import Gaussian
from utils.visual import GifConverter

from typing import Dict


class DualDiscreteHawkes:
    def __init__(self,
                 bandwidth: float,
                 init_mu0: float=None,
                 init_A: float=None,
        ...
        self.init_mu0, self.init_A, self.init_B = init_mu0, init_A, init_B
    ...
    def init_params(self,
        ...
        g_t_prime = DiscreteHawkes._g_initializer(num_sample, init_exp_lambda)
        g_t_prime = normalize(g_t_prime)
        return mu0, A, B, mu_t, g_t, g_t_prime
    ...
    def load_model(self,
                   ...
                   mu_t: np.ndarray,
                   g_t: np.ndarray,
                   g_t_prime: np.ndarray,
                   truncated: bool=False) -> Callable:
        _, lamdba_ = self._eval_fn_factory(
                X, Y,
                mu0, A, B,
                mu_t, g_t, g_t_prime,
                truncated
        ...
        internal_g_lag = self._g_lag_factory(X, g_t)
        external_g_lag = self._g_lag_factory(Y, g_t_prime)

        return LambdaModel(
            lambda_fn=lamdba_,
            bg_fn=lambda x: mu0 * mu_(x),
            internal_excite_fn=lambda x: A * internal_g_lag(x),
            external_excite_fn=lambda x: B * external_g_lag(x),
        )
    ...
    def fit(self,
        ...
        mu0, A, B, mu_t, g_t, g_t_prime = self.init_params(
                X, 
                self.init_mu0, 
                self.init_A,
                self.init_B
        ...
        if verbose:
            print('[Init]')
            print('mu0: {:.7f}\tA: {:.7f}\tB :{:.7f}'.format(mu0, A, B))
        ...
        if self._save_progress:
            ...
            self.gif_converter_dict['g_t_prime'].push(
                range(num_sample), g_t_prime, 
                'Init'
            )
        ...
        for idx in range(epoch):
            ...
            mu0, A, B, mu_t, g_t, g_t_prime = self._fit_impl(
                X, Y, 
                mu0, A, B, 
                mu_t, g_t, g_t_prime,
                smooth_lambda_normalizer,
    ...
    def _eval_fn_factory(self,
        ...
        internal_g_lag = self._g_lag_factory(X, g)
        external_g_lag = self._g_lag_factory(Y, g_prime)
        ...
        def _lambda(x):
            # x is idx 
            lambda_res = mu0 * mu[x] + A * internal_g_lag(x) + B * external_g_lag(x)
            if truncated:
    ...
    def _fit_impl(self,
        ...
        updated_mu, updated_g, updated_g_prime = self._E_step(
            X, Y,
            mu0, A, B,
            mu_, g_, g_prime_,
            smooth_lambda_normalizer,
        ...
        updated_mu0, updated_A, updated_B = self._M_step(
            X, Y,
            mu0, A, B,
            updated_mu,
            updated_g,
        ...
        if verbose:
            print('mu0: {:.7f}\tA: {:.7f}\tB: {:.7f}'.format(updated_mu0, updated_A, updated_B))

        return updated_mu0, updated_A, updated_B, updated_mu, updated_g, updated_g_prime
    ...
    def _E_step(self,
                ...
                smooth_lag_normalizer: np.ndarray):
        occurence = np.argwhere(X == 1).reshape(-1)
        Y_occur = np.argwhere(Y == 1).reshape(-1)

        self_occur_lag = pairwise_difference(occurence) # 1-d array
        mutual_occur_lag = pairwise_difference(occurence, Y_occur)
        ...
        Z_g = self.pairwise_kernel_est(num_sample, self_occur_lag)
        num_occur = len(occurence)

        flattened_occur = np.repeat(occurence[None,:], repeats=num_occur, axis=0)[np.triu_indices(num_occur, k=1)]
        rho_ij = eval_g(self_occur_lag) / eval_lambda(flattened_occur)  # dask array
        ...
        # get occurence count
        Z_g_prime = self.pairwise_kernel_est(num_sample, mutual_occur_lag)
        mutual_lag_mat = occurence[None,:] - Y_occur[None,:]
        _, valid_occur = np.where(mutual_lag_mat > 0)
        omega_ij = eval_g(mutual_occur_lag) / eval_lambda(valid_occur)  # dask array

        updated_g_prime = da.sum(
            omega_ij * Z_g_prime,
            axis=-1
        ).compute()
        ...
        updated_g_prime = normalize(updated_g_prime)

        return updated_mu, updated_g, updated_g_prime
    ...
    def _M_step(self,
        ...
        for _ in range(iter_round):
            # update mu0
            eval_phi, eval_lambda = self._eval_fn_factory(
                    X, Y,
                    mu0, A, B,
                    mu, g, g_prime,
                    truncated
            ...
            other_excite = (mu0 * eval_mu(occurence) + B * external_g_lag(occurence)) / eval_lambda(occurence)
            A = sum(1 - other_excite) / \
                sum(internal_g_lag(non_occur) / (1-eval_lambda(non_occur)))
            ...
            other_excite = (mu0 * eval_mu(occurence) + A * internal_g_lag(occurence)) / eval_lambda(occurence)

            # update B
            B = sum(1 - other_excite) / \
                sum(external_g_lag(non_occur) / (1-eval_lambda(non_occur)))
...
class LambdaModel:
    def __init__(self,
                 lambda_fn: Callable,
                 bg_fn: Callable,
                 internal_excite_fn: Callable,
                 external_excite_fn: Callable
                 ):
        self.lambda_fn = lambda_fn
        self.bg_fn = bg_fn
        self.internal_excite_fn, self.external_excite_fn = internal_excite_fn, external_excite_fn

    def __call__(self, x):
        return self.lambda_fn(x)

    def internal_excit(self, x):
        return self.internal_excite_fn(x)

    def external_excit(self, x):
        return self.external_excite_fn(x)
